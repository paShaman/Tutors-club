...
| Errors | **GlitchTip** (Sentry-совместимый) via `sentry/sentry-laravel`: `config/sentry.php`, DSN из `GLITCHTIP_DSN`, канал `sentry` в `config/logging.php` |
...
- **Ошибки уходят в GlitchTip.** Необработанные исключения репортит `Sentry\Laravel\Integration::handles()` в `bootstrap/app.php`, `Log::error()` и выше — канал `sentry` в `config/logging.php` (исключения из логов пропускаются через `report_exceptions => false`, чтобы не было дублей). DSN проекта — `GLITCHTIP_DSN`; без валидного DSN SDK молча выключен. Не вызывайте SDK напрямую и не добавляйте второй канал/логгер — код ошибок пишите через `Log::error(...)`.
...
1. Create `.env` manually — **there is no `.env.example`** and `.env` is gitignored. Include at least `APP_KEY` (generate via `php artisan key:generate`), `APP_URL`, DB settings (`DB_CONNECTION=mysql`, `DB_DATABASE/HOST/PORT/USERNAME/PASSWORD`), Redis (`CACHE_DRIVER=redis`, `SESSION_DRIVER=redis`, `REDIS_HOST/PORT/PASSWORD`, `REDIS_CACHE_DB=1`, `REDIS_DB=0`), and optional feature keys: `VK_ID_APP_ID`, `VK_ID_REDIRECT_URL`, `YANDEX_CLIENT_ID`, `YANDEX_CLIENT_SECRET`, `YANDEX_REDIRECT_URI`, `SMARTCAPTCHA_SITE_KEY`, `SMARTCAPTCHA_SERVER_KEY`, `LESSON_DEFAULT_PRICE`, `LESSON_DEFAULT_DURATION`, `YANDEX_METRIKA_ID`, error tracking (`GLITCHTIP_DSN` — DSN проекта GlitchTip вида `https://<key>@<host>/<project-id>`; опционально `SENTRY_ENVIRONMENT`, `SENTRY_RELEASE`, `SENTRY_LOG_LEVEL`), feedback bots (`TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, `TELEGRAM_BOT_USERNAME`, `TELEGRAM_WEBHOOK_SECRET`, `MAX_BOT_TOKEN`, `MAX_OWNER_ID`, `MAX_BOT_USERNAME`, `MAX_WEBHOOK_SECRET`, `FEEDBACK_PRIMARY_BOT`), and the personal tutor bot (`TELEGRAM_STUDENTS_BOT_TOKEN`, `TELEGRAM_STUDENTS_BOT_USERNAME`, `TELEGRAM_STUDENTS_WEBHOOK_SECRET`). Прокси для исходящих запросов к мессенджерам задаётся опционально (`TELEGRAM_PROXY` — общий для Telegram, `TELEGRAM_STUDENTS_PROXY` — только для бота учеников, `MAX_PROXY` — для MAX; формат `http://user:pass@host:port` или `socks5h://host:port`); он нужен, когда хостинг не может открыть соединение с `api.telegram.org` (в логах `cURL error 28`). Пустое значение = запросы идут напрямую: опция прокси не передаётся вовсе, поэтому переменные окружения хостинга (`HTTPS_PROXY`, `ALL_PROXY`), которые читает сам Guzzle, продолжают работать как раньше. Отдельного флага включения не нужно — значение само является переключателем.
